...
// The default endurance window intentionally exceeds Playwright's global
// 30-second setting in playwright.config.ts.
test.setTimeout(DURATION + 20_000);
...
test("memory leak — heap growth stays under budget over a long-running room", async ({
  ...
  await ctx.addInitScript(
    ...
    { prefix: "mesh-queue", room: `leak-${Date.now()}` },
  );

  const a = await ctx.newPage();
  const b = await ctx.newPage();
  await Promise.all([
    a.goto("/mesh-queue/", { waitUntil: "domcontentloaded" }),
    b.goto("/mesh-queue/", { waitUntil: "domcontentloaded" }),
  ]);
